...
与其他 Akka 模块类似，Akka HTTP 通过 [Typesafe Config](https://github.com/typesafehub/config) 进行配置。Akka 模块提供默认的参考配置，若应用的配置与默认不同，则要提供 `application.conf` 文件，内容为所有与默认配置不同的配置项。

下面是 `akka-http-core` 的默认配置：
...
除 `akka-http-core` 和 `akka-http` 以外，其他模块未提供 `Typesafe Config` 配置。
